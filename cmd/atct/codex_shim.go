...
type codexShimGlobalOptionKind uint8

const (
	codexShimGlobalOptionValue codexShimGlobalOptionKind = iota
	codexShimGlobalOptionBoolean
)

var codexShimGlobalOptions = map[string]codexShimGlobalOptionKind{
	"-c":                      codexShimGlobalOptionValue,
	"--config":                codexShimGlobalOptionValue,
	"--enable":                codexShimGlobalOptionValue,
	"--disable":               codexShimGlobalOptionValue,
	"--remote":                codexShimGlobalOptionValue,
	"--remote-auth-token-env": codexShimGlobalOptionValue,
	"-i":                      codexShimGlobalOptionValue,
	"--image":                 codexShimGlobalOptionValue,
	"-m":                      codexShimGlobalOptionValue,
	"--model":                 codexShimGlobalOptionValue,
	"--local-provider":        codexShimGlobalOptionValue,
	"-p":                      codexShimGlobalOptionValue,
	"--profile":               codexShimGlobalOptionValue,
	"-s":                      codexShimGlobalOptionValue,
	"--sandbox":               codexShimGlobalOptionValue,
	"-C":                      codexShimGlobalOptionValue,
	"--cd":                    codexShimGlobalOptionValue,
	"--add-dir":               codexShimGlobalOptionValue,
	"-a":                      codexShimGlobalOptionValue,
	"--ask-for-approval":      codexShimGlobalOptionValue,
	"--strict-config":         codexShimGlobalOptionBoolean,
	"--oss":                   codexShimGlobalOptionBoolean,
	"--approve-for-me":        codexShimGlobalOptionBoolean,
	"--dangerously-bypass-approvals-and-sandbox": codexShimGlobalOptionBoolean,
	"--dangerously-bypass-hook-trust":            codexShimGlobalOptionBoolean,
	"--search":                                   codexShimGlobalOptionBoolean,
	"--no-alt-screen":                            codexShimGlobalOptionBoolean,
}

func codexShimCommandAfterGlobalOptions(args []string) (string, bool) {
	for index := 0; index < len(args); {
		arg := args[index]
		option := arg
		if strings.HasPrefix(arg, "--help=") || strings.HasPrefix(arg, "--version=") {
			return arg, true
		}
		if strings.HasPrefix(arg, "--") {
			if name, value, hasValue := strings.Cut(arg, "="); hasValue {
				kind, ok := codexShimGlobalOptions[name]
				if !ok || kind != codexShimGlobalOptionValue || value == "" {
					return "", false
				}
				index++
				continue
		...
		kind, ok := codexShimGlobalOptions[option]
		if !ok {
			return arg, true
		}
		if kind == codexShimGlobalOptionBoolean {
			index++
			continue
		}
		if index+1 >= len(args) {
...
func codexShimPassesThrough(args []string) bool {
	...
	first, ok := codexShimCommandAfterGlobalOptions(args)
	if !ok {
		return false
	}
	switch first {
