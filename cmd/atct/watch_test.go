...
func TestReconcileWatchScopeRoutesRecoveryToCommanderOncePerGeneration(t *testing.T) {
	body := `{"goals":[],"decisions":[],"goal_handoffs":[],"plan_handoffs":[],"task_handoffs":[],"recoveries":[{"condition":"monitor_missing","target_role":"commander","project_id":1,"goal_id":42,"handoff_id":"h1","scope_key":"goal:42:subcommander:h1","generation":"received-1","expected_role":"subcommander","instruction":"start the rightful monitored wrapper with atct codex monitor --role subcommander --goal 42 --"}]}`
	client := &http.Client{Transport: watchRoundTripper(func(req *http.Request) (*http.Response, error) {
		if req.URL.Path != "/api/events/reconcile" {
			return nil, fmt.Errorf("unexpected request path %q", req.URL.Path)
		}
	...
	for range 2 {
		...
		if err := reconcileWatchScope(
			context.Background(), client, "http://daemon", watchScope{Role: "commander", ProjectID: "1"}, &output,
			delivered, &lastWakeupContent, wakeupDiscrepancyDelivered, detectionDelivered,
			newWatchPassThroughFilter(), nil, actionSink,
		); err != nil {
			t.Fatalf("commander reconcileWatchScope: %v", err)
		}
	}
	wantLine := "atct orchestration recovery: monitor_missing (target_role commander, scope goal:42:subcommander:h1): start the rightful monitored wrapper with atct codex monitor --role subcommander --goal 42 --\n"
	if got := output.String(); got != wantLine {
		t.Fatalf("commander recovery output = %q, want one generation", got)
	}
	...
	lastWakeupContent := ""
	if err := reconcileWatchScope(
		context.Background(), client, "http://daemon", watchScope{Role: "executor", ProjectID: "1", GoalID: "42", TaskID: "7"}, &childOutput,
		make(map[watchDeliveryKey]struct{}), &lastWakeupContent,
		make(map[watchWakeupDeliveryKey]struct{}), make(map[watchDetectionDeliveryKey]struct{}),
		newWatchPassThroughFilter(), nil, actionSink,
	); err != nil {
		t.Fatalf("executor reconcileWatchScope: %v", err)
	}
	if childOutput.Len() != 0 || len(actions) != len(wantActions) {
		t.Fatalf("executor received commander recovery: output=%q actions=%#v", childOutput.String(), actions)
	}
}

func TestEmitWatchRecoveryDeliversChangedGenerationOnce(t *testing.T) {
	var output bytes.Buffer
	delivered := make(map[watchDeliveryKey]struct{})
	wakeupDiscrepancyDelivered := make(map[watchWakeupDeliveryKey]struct{})
	detectionDelivered := make(map[watchDetectionDeliveryKey]struct{})
	for _, generation := range []string{"generation-1", "generation-1", "generation-2"} {
		if err := emitWatchDecisionWithState(&output, "orchestration.recovery", watchDecision{
			Condition:   "monitor_missing",
			TargetRole:  "commander",
			ScopeKey:    "task:7:executor:h1",
			Generation:  generation,
			Instruction: "restart monitored executor",
		}, delivered, nil, wakeupDiscrepancyDelivered, detectionDelivered); err != nil {
			t.Fatalf("emitWatchDecision(%s): %v", generation, err)
		}
	}
	want := "atct orchestration recovery: monitor_missing (target_role commander, scope task:7:executor:h1): restart monitored executor\n" +
		"atct orchestration recovery: monitor_missing (target_role commander, scope task:7:executor:h1): restart monitored executor\n"
	if got := output.String(); got != want {
		t.Fatalf("recovery output = %q, want one line per generation", got)
	}
}
