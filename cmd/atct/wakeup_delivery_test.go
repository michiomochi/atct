...
func TestEmitWatchDecisionSuppressesUnchangedWakeupAndEmitsChangedContent(t *testing.T) {
	...
	want := []string{
		"atct wakeup: actionable_goals=4 unassigned_goals=0 unstarted_tasks=4 waiting_answer_tasks=0 untouched_tasks=4 delegated_tasks=0 waiting_answers=24 unassigned=[]",
		"atct wakeup: actionable_goals=4 unassigned_goals=0 unstarted_tasks=3 waiting_answer_tasks=0 untouched_tasks=3 delegated_tasks=0 waiting_answers=24 unassigned=[]",
	}
	if len(got) != len(want) {
...
func TestEmitWatchDecisionKeepsWakeupDeliveryPerWatch(t *testing.T) {
	...
	want := "atct wakeup: actionable_goals=1 unassigned_goals=0 unstarted_tasks=1 waiting_answer_tasks=0 untouched_tasks=1 delegated_tasks=0 waiting_answers=0 unassigned=[]\n"
...
func TestEmitWatchDecisionEmitsWakeupWhenContentReturns(t *testing.T) {
	...
	want := strings.Join([]string{
		"atct wakeup: actionable_goals=4 unassigned_goals=0 unstarted_tasks=4 waiting_answer_tasks=0 untouched_tasks=4 delegated_tasks=0 waiting_answers=0 unassigned=[]",
		"atct wakeup: actionable_goals=4 unassigned_goals=0 unstarted_tasks=3 waiting_answer_tasks=0 untouched_tasks=3 delegated_tasks=0 waiting_answers=0 unassigned=[]",
		"atct wakeup: actionable_goals=4 unassigned_goals=0 unstarted_tasks=4 waiting_answer_tasks=0 untouched_tasks=4 delegated_tasks=0 waiting_answers=0 unassigned=[]",
	}, "\n") + "\n"
	if out.String() != want {
