...
### 4. 「どちらの handoff も無い」ときは通す

門番は `task handoff の保持者 or goal handoff の保持者` だが、**open な handoff が
どちらも無いタスクは通す。**

理由: **所有が記録されていないタスクを拒むと、handoff を使わない単独作業の経路が動かなくなる。**
`atct:start` で 1 セッションが宣言・claim・実装まで通す形がこれにあたる。
**守るべき保持者が居ないときに、守る対象は無い。**プロジェクト所属の確かめは
`internal/daemon/handler.go` の `ensureAgentSessionProject` が引き続き行う。

**`ClaimTask` は claimer を `received_by` とする task handoff を張る**ので、
claim 済みのタスクは「所有が記録されている」側に入る。**claim した瞬間から、内容を直せるのは
その claimer とゴールの保持者だけになる。**

### 5. 門番は `files` だけでなく `UpdateTaskContent` 全体にかける

理由 2 つ。**`files` と辻褄を合わせるべき `description` を他人が書き換えられる穴を残さない。**
フィールドごとに門番を分けると、拒否の理由が「どのフィールドを渡したか」に依存して揺れる。

**結果として、この実装はゴール 139（`files` による直列化機構の削除）の後も残る。**
門番は「タスクの内容を誰が直せるか」であって、`files` の直列化とは独立である。

### 6. 判定順は 引数検査 → 門番 → status

`done` のタスクに保持者でないセッションが来たときの拒否理由を一意にするため。
**門番は「そもそも触ってよいか」、status は「いま触れる状態か」。**

`ErrTaskNotFound` の経路だけは門番より前に出る必要がある。門番が `GetTaskGoalID` を引くので、
存在しない task ID が `sql.ErrNoRows` に化ける。**門番の中で `ErrTaskNotFound` に変換する。**

### 7. `atct_task_update_content` の `Description` を直す

**このゴールは「説明文を型の代わりに読んだ」ことから生まれている。**そのうえ今回の門番は
**入力スキーマから見えない拒否**を新たに作る。同じ形を残さないために 1 行だけ書き換えた。

## 決定 2 の続き: 完了条件 4 は人間が落とした

**decision 409（2026-08-27 02:00 回答済み）:**

> worktree がはいって files は不要になったのでけして

完了条件 4（書き換えた `files` で `like '%<path>%'` が当たることを実測する）と
タスク 653 は**落とした。**ゴール 139 が直列化機構そのものを削除する。

**残る 650 / 651 / 652 は直列化と独立である。**タスクの内容を保持者以外が
書き換えられないことは、`files` が消えても要る。
