...
**タスク `task 587` の成果物。**稼働版 0.54.0 で commander と subcommander の両方が
これで止まったため調べた。**判断ではなく観測の記録である。**未確定の点は末尾に残してある。

---

対象タスク: `task 587-a5c9-402d-9206-b613227ec48e`
...
```sh
sqlite3 ~/.atct/atct.db '.backup /tmp/atct-task 587.db'
```
...
```
atct_project_release(<project_id>)   保持者でなくても通る（タスク task 588）
...
**当初 commander は「生存の指標を `last_seen_at` に変える」を推奨し、決定 `decision 254` として
出した。人間の指摘で取り下げた。**
...
**先に決めるべきは同一性で、生存はその後である。**立て直した問いは決定 `decision 267`。
...
`task 567` の本文は候補③（pid を使わない）を検討するとき「いまの `pid 0` が 9 件あるのも
...
（この記録は決定 `decision 253` に置かれていたが、**既定の無い決定がタスクを永久に
`done` にできなくする**ため、ここへ移して取り下げた。同じ形が 14 件ある——`task 605` を参照。）
...
09:18:00.652494  bf866697  atct-goal 115-executor      ← identify 後。同じ id、同じ registered_at
```
...
**何が 2 回登録しているかは依然として不明である。**`task 587` の完了条件 (2) は未達。
**推測で埋めない。**
...
（`handoff-dotfiles-goal 116-01` から `-04`）。うち 2 回は MCP 再接続、1 回は daemon
