...
**検知の経路から外部プロセスを起こすので、期限を付ける。**`handoffWorktreeActivity` が
受け取る ctx は `tickerCtx`（`internal/daemon/server.go:154` の `context.WithCancel`）で、
**daemon の寿命と同じで Deadline を持たない。**期限を付けないと、`git status` が詰まった
worktree 1 つで 30 秒周期のスイープごと止まる。**ゴール 91 が直した「1 件の失敗で全件が
落ちる」形に戻り、しかも `continue projectLoop` の手前で止まるので `continue` に到達しない。**
詰まる筋は実在する（`.git/index.lock` の残骸、ネットワーク FS）。

期限は**関数 1 回あたり合計 3 秒**にする。呼び出しごとではない。未報告の handoff が
複数あるときにスイープが線形に伸びるのを避けるためである。

**件数の上限は置かない。**`os.Stat` は `git status` が既に列挙した分だけの有界な仕事で、
重いのは git のほうである。**件数で打ち切ると「変更が無い」と読める結果を黙って作る。**
期限で切れば打ち切りは必ず「不明」に落ちるので、`unchanged` と混ざらない。
そのため stat のループの中でも `ctx.Err()` を見る。

**検査の重心は「活動があれば付く」側ではなく「git が失敗しても検知そのものは出る」側に置く。**
ここが落ちると、git の不調で検知が消える。
