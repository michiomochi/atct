...
- 節の境界は `^## ` と `^### ` の両方。`### Two-layer delegation` は
  `## Delegate a task` とは別の節として数える。それぞれ独自の番号列と
  `**Out of order:**` を持つ。
- 番号列は 1 節につき 2 系統ある。行頭の `^[0-9]+\. `（本文）と
  `^> [0-9]+\. `（引用ブロック）である。**引用ブロックは subcommander へ丸ごと
  転記される文言なので、そこの番号が崩れても本文の検査では見つからない。**
  D3 で `## Delegate a goal` の引用ブロックに 4 段が入ったので、両方を検査する。
- **A1・A2（連続性・2 項目以上）は `skills/atct/SKILL.md` と
  `skills/start/SKILL.md` の両方に適用する。**
- **A3（`**Out of order:**` の要求）は `skills/atct/SKILL.md` の本文番号列にだけ
  適用する。**引用ブロックの番号列には要求しない（その節の本文側が既に持つ）。
  `skills/start/SKILL.md` にも要求しない——`## The loop` は 6 に達したら 1 に戻る
  循環であって一方向の順序ではなく、逆順という状態が無い。
...
**ゴール 192（handoff の状態遷移を機構で守る）が `## Delegate a goal` と
`## Where an unsent report goes` を触っている。**192 は同じ順序を機構で守るゴールなので、
`## Delegate a goal` に順序の説明を重複して書く可能性がある。
**192 が main に着地したら `git merge main` し、同じことが 2 か所に書かれていないか
確かめること。**このゴールは `## Where an unsent report goes` を触らない。
