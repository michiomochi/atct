...
**順序を守る。**この 3 つを足し、10 種類すべてが届くことを稼働版で実測し、それから
Stop hook を消す。逆にすると、その間だけ「人間の回答が届かない」穴が空く。
**1 番は回答が失われる形なので最も落とせない。**

`plugin/hooks/hooks.json` から `Stop` を外して `plugin/hooks/stop` を削除する。
**`pre-ask` と `session-start` は消さない。**`atct pending` はコマンドとして人間も
叩けるので、フックを消してもコマンドは残す。

**dotfiles 側は 2 つの `apply` を待っている** ── `single-subcommander.sh` と、
役割定義（`b66acc5`）。どちらも人間の承認が要る。
