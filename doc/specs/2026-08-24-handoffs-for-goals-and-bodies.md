...
今日 `task 550` で「handoff を自分宛に作る」案を**形式的だとして却下した**が、
...
実測（2026-08-24）: タスク `task 541` は executor-13 が着手し、圧縮 5 回で pane を
...
`goal 111`（UUID をやめて連番 ID）が 8 表を作り直す予定で、`goal 88`（外部キーを
...
タスク `task 569` として切り出した。

## 7. 訂正: 短い本文は通る（2026-08-24・同日の追加実測）

6 節は「本文は安全ゲートを通らない」と結論したが、**言い過ぎだった。**

**同じ日に executor-31 が 475 文字の完了報告を書き、通っている。**

```
拒否された（6 節）  4,547 文字 / 絶対パス / 差分 / 失敗したテストの出力
通った              475 文字 / 相対パス / ツール名 / 合否だけ
```

**ゲートは宛先ではなく中身を見ている**——ここは 6 節のとおりである。
**しかし中身の何を見ているかを 1 例から一般化していた。**

### 境界は長さと具体性にある

通ったほうの本文は、**何をどのファイルに入れたか**と**どの検証が通ったか**を書いており、
**リポジトリの中身そのもの（差分・失敗出力・絶対パス）を運んでいない。**

**したがって `complete_report` は使える。**列は空のままにならない。

### さらに重要なこと

**executor-31 は commander に言われる前に自分で呼んでいた。**
`d4229c4` でスキルの手順に「完了を記録しろ」を入れた効果である。
**同じ日に、ターミナルマルチプレクサ経由の報告は 2 段で止められていた:**

```
1. sandbox が aqua の metadata 更新を拒否
2. 権限昇格後も「外部宛てメッセージ送信」として安全審査が停止
```

**atct の MCP は外部プロセスを起動しないので、その両方に当たらない。**
報告経路として、ターミナルマルチプレクサより atct のほうが確実である。

### 残る穴

**委譲した側に届く経路が無い。**`handoff_unreported` は報告が**無い**ときに鳴るが、
**報告が届いたときに鳴るものが無い。**タスク `task 574` として切り出した。
