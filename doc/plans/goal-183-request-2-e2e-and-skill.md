...
2026-08-27 から 08-28 の実測が 9 件ある。引き金は 4 つで、**handoff がどうなるかが違う。**

| 引き金 | handoff |
|---|---|
| daemon の再起動（**版が上がったかとは無関係**に起きる） | 開いたまま。落ちたのはセッション記録だけ |
| **transport とセッションキーの対応が失われる**（原因未測定） | 開いたまま。goal handoff も DB のセッション行も生きている |
...
（commander に再発行を頼む）が要るのは最後の行だけである。

**2 行目の原因は未測定である。「MCP transport の再接続」などと断定して書くな。**
書いてよいのは「handoff が閉じていないのに役割が落ちることがある」という外形だけである。
