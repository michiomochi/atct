...
make_archives() {
  ...
  local atct_archive="$fixture_dir/atct_0.3.0_darwin_arm64.tar.gz"
...
test_static_contract() {
  ...
  assert_file_contains '"version": "0.3.0"' "$REPO_ROOT/.claude-plugin/plugin.json"
  assert_file_contains 'VERSION="0.3.0"' "$REPO_ROOT/bin/_resolve"
...
test_download_cache_and_mcp_stdout() {
  ...
  assert_file_contains 'https://github.com/michiomochi/atct/releases/download/v0.3.0/atct_0.3.0_darwin_arm64.tar.gz' "$curl_log"
  assert_file_contains 'https://github.com/michiomochi/atct/releases/download/v0.3.0/checksums.txt' "$curl_log"
  [[ -x "$home/.atct/bin/atct-0.3.0" ]] || fail 'versioned atct cache is missing'

  HOME="$home" PATH="$fake_bin:$PATH" FIXTURES_DIR="$fixtures" CURL_LOG="$curl_log" FAKE_OS=Darwin FAKE_ARCH=arm64 "$REPO_ROOT/bin/atct-mcp" </dev/null >"$mcp_stdout" 2>"$mcp_stderr"
  assert_empty_file "$mcp_stdout"
  assert_file_contains 'fake mcp' "$mcp_stderr"
  [[ -x "$home/.atct/bin/atct-mcp-0.3.0" ]] || fail 'versioned atct-mcp cache is missing'
...
test_checksum_failure() {
  ...
  [[ ! -e "$home/.atct/bin/atct-0.3.0" ]] || fail 'checksum mismatch left an executable cache'
}
