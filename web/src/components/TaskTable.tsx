...
function TaskRelease({ task, onRefresh }: { task: TaskView; onRefresh: () => void }) {
  ...
  return (
    <div>
      ...
      {error && <p className="mt-2 max-w-48 text-base text-danger-700" role="alert">{error}</p>}
    </div>
  );
}
...
function ClaimCell({ task }: { task: TaskView }) {
  ...
  return (
    <div className="space-y-1 text-ink-700">
      ...
      <p className="text-base text-ink-500">{task.claimed_by ? formatDuration(locale, task.held_for_seconds) : t("task.claim.unclaimed")}</p>
    </div>
  );
}
...
function TaskTitle({ task }: { task: TaskView }) {
  return (
    <div>
      ...
      <p className="mt-1 font-mono text-base text-ink-500">{task.id}</p>
    </div>
  );
}
...
export function TaskTable({ tasks, mode, onRefresh }: Props) {
  ...
  if (mode === "goal") {
    ...
    return (
      <div className="table-scroll">
        <Table className="min-w-[52rem] w-full border-collapse text-left text-base">
          ...
          <Table.Body>
            {orderedTasks.map((task, index) => (
              <Table.Row className="border-b border-line align-top last:border-b-0" key={task.id}>
                ...
                <Table.Cell className="px-3 py-4 text-ink-700">{taskStatusLabel(locale, task.status, task.open_decisions?.length ?? 0, task.claimed_by)}</Table.Cell>
  ...
  return (
    <div className="table-scroll">
      <Table className={needsDecision ? "min-w-[70rem] w-full border-collapse text-left text-base" : "min-w-[52rem] w-full border-collapse text-left text-base"}>
