  ...
  return (
    <section className="border-t border-line pt-5" data-testid="decision-history" aria-labelledby="decision-history-heading">
      <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-2">
        ...
        {omittedCount > 0 && <p className="text-base text-ink-700">{t("goal.history.omitted", { count: omittedCount })}</p>}
      </div>
                ...
                <Table.Row className="border-b border-line align-top last:border-b-0" key={decision.decision_id}>
                  ...
                  <Table.Cell className="px-3 py-4 text-ink-700">
                    {revisingDecisionID === decision.decision_id ? (
                      ...
                      >
                        ...
                        <input
                          ...
                          className="w-full rounded border border-line px-2 py-1 text-base"
                        ...
                        <div className="flex gap-2">
                          <button
                            className="rounded bg-ink-950 px-2 py-1 text-base font-semibold text-white disabled:opacity-50"
                            type="submit"
                            disabled={revisionSubmitting}
                          >
                          ...
                          <button
                            className="rounded border border-line px-2 py-1 text-base font-semibold text-ink-700"
                            type="button"
                            onClick={() => {
                              setRevisingDecisionID(null);
                              setRevisionOptions("");
                      ...
                      <button
                        className="rounded border border-line px-2 py-1 text-base font-semibold text-ink-700 hover:border-ink-500"
                        type="button"
                        onClick={() => {
                    ...
                    {revisionMessage?.decisionID === decision.decision_id && (
                      <p className={`mt-2 text-base ${revisionMessage.error ? "text-red-700" : "text-ink-700"}`} role="status">
